...
## 가장 중요했던 판단 — 가설보다 데이터에 맞춰 질문을 다시 정의

처음에는 청년 고용 문제를 **취업이 계속 어려워지는 현상**으로 보고 실업률·고용보조지표·구직단념자 등 기초 지표부터 확인했습니다. 그러나 일부 지표는 처음 예상한 방향과 다르게 나타났고, 기존 가설을 유지한 채 분석을 이어가면 데이터가 보여주는 문제보다 원하는 결론을 먼저 정하게 될 수 있다고 판단했습니다.

그래서 질문을 **“청년은 왜 취업하기 어려운가?”에서 “취업한 청년은 어떤 노동 경험을 하고 있는가?”**로 바꿨습니다. 이후 KWCS 2017·2023을 이용해 노동의 질을 7개 하위지표로 구성하고, 청년 내부의 노동유형과 취약집단을 구분한 뒤 관련 조건을 확인했습니다.

```text
초기 가설 확인
→ 거시 고용지표 EDA
→ 예상과 다른 흐름 확인
→ 문제를 취업 여부에서 취업 이후 경험으로 재정의
→ 노동의 질 지표 설계
→ 노동유형·취약집단·관련 조건 분석
```

이 과정에서 분석기법을 많이 적용하는 것보다 **데이터가 처음 가설과 다를 때 질문 자체를 다시 정의하는 판단**이 최종 메시지를 더 명확하게 만든다는 점을 확인했습니다.
...
## 9. Policy / Practical Use Case

현재 분석은 정책의 인과효과를 추정한 것이 아니라 **어떤 노동 경험을 지속적으로 모니터링하고 어떤 집단을 우선 점검할지 정하는 진단 근거**로 활용할 수 있습니다.
...
→ 취약 노동유형 및 관련 조건 식별
→ 취약 원인에 맞는 지원 방향 검토
→ 이후 조사에서 변화 추적 및 별도 효과평가
```

### 1. 모니터링 지표

취업률과 함께 노동의 질 지수와 7개 하위지표를 연도·고용형태·연령집단별로 추적합니다. 특히 이번 분석에서 하락폭이 컸던 **고용안정성(-14.7%)**과 **재취업 가능성(-4.5%)**을 우선 모니터링 지표로 볼 수 있습니다.

### 2. 우선 점검 대상

청년 전체를 하나로 보지 않고 **의미추구·불안정형**, **저만족·저품질형** 등 취약 노동유형을 별도로 확인합니다. 회귀분석에서 취약 노동유형과 양의 관련을 보인 임시직·일용직 등의 조건도 추가 점검 기준으로 활용할 수 있습니다.

### 3. 정책·지원 방향

- 고용안정성이 낮은 집단: 고용 지속성 및 계약 안정 관련 지원 검토
- 재취업 가능성이 낮은 집단: 직무전환·재취업·역량개발 지원 검토
- 장시간·불균형형: 근로시간과 일·생활 균형 개선 방향 검토
- 저만족·저품질형: 만족도 하나가 아니라 전반적 근로환경을 함께 점검

### 4. 효과 검증

현재 결과만으로 특정 지원정책이 효과적이라고 주장할 수 없습니다. 실제 정책 시행 후에는 반복 조사, 시행 전·후 비교, 가능한 경우 적절한 비교집단을 활용해 **정책 개입과 노동의 질 변화의 관계를 별도로 평가**해야 합니다.
